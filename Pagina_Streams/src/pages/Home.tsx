...
export default function InicioLogeado({
  sidebarAbierto = true,
}: InicioLogeadoProps) {
  const [streamerDestacado, setStreamerDestacado] =
    useState<StreamerData | null>(null);
  const [streamersRecomendados, setStreamersRecomendados] = useState<
    StreamerData[]
  >([]);
      ...
      try {
        ...
        if (result.success && result.data) {
          if (Array.isArray(result.data) && result.data.length > 0) {
            ...
            console.log(
              `✅ [Home] Streamer destacado: ${result.data[0].NombreUsuario}`
            );
          } else if (!Array.isArray(result.data) && result.data.NombreUsuario) {
            setStreamerDestacado(result.data);
            console.log(
              `✅ [Home] Streamer destacado: ${result.data.NombreUsuario}`
            );
          } else {
    ...
    const cargarStreamersRecomendados = async () => {
      ...
      try {
        ...
        if (result.success && result.streamers) {
          ...
          console.log(
            `✅ [Home] ${result.streamers.length} streamers cargados`
          );
        } else {
  ...
  return (
    <div className="home-page" style={{ paddingLeft }}>
            ...
            <>
              ...
              <div
                style={{
                  ...
                  padding: "20px",
                  background:
                    "linear-gradient(135deg, #9147ff 0%, #6441a5 100%)",
                  borderRadius: "12px",
                }}
              >
                <img
                  src={
                    streamerDestacado.ImagenPerfil ||
                    `https://ui-avatars.com/api/? name=${streamerDestacado.NombreUsuario}&background=9147ff&color=fff&size=80`
                  }
            ...
            >
              <div className="hero-grid__card-top">
                <img
                  src={
                    streamerDestacado.ImagenPerfil ||
                    `https://ui-avatars.com/api/?name=${streamerDestacado.NombreUsuario}&background=random&size=50`
                  }
                  alt={streamerDestacado.NombreUsuario}
      ...
      <section>
        <h2 className="section-title">🎯 Canales más vistos</h2>
          ...
          <>
            <div className="recommendations">
              {streamersRecomendados.map((streamer, index) => (
                <div
                  key={`${streamer.NombreUsuario}-${index}`}
                    ...
                    <div
                      ...
                      style={{
                        backgroundImage: `url(${
                          ...
                          `https://ui-avatars.com/api/?name=${streamer.NombreUsuario}&background=random&size=200`
                        })`,
                    ...
                    <div className="recommendations__meta">
                      <Link to={`/perfil/${streamer.NombreUsuario}`}>
                        <img
                          src={
                            streamer.ImagenPerfil ||
                            `https://ui-avatars.com/api/?name=${streamer.NombreUsuario}&background=random&size=40`
                          }
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div
                          className="recommendations__channel"
                          style={{ color: "#adadb8" }}
                        >
                        ...
                        <div
                          ...
                          style={{ color: "#adadb8" }}
                        >
                          {streamer.ViendoCount} espectadores ·{" "}
                          {streamer.TotalChats} chats totales
                        </div>
